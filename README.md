...
CREATE TABLE SuperBowlWinners(Winner TEXT, Wins INTEGER);
INSERT INTO SuperBowlWinners SELECT Winner, COUNT(Winner) AS superbowlcount FROM superbowls GROUP BY Winner;


SELECT Winner, MAX(Wins) FROM SuperBowlWinners;
SELECT AVG(Wins) AS Average_Wins FROM SuperBowlWinners;
SELECT Winner FROM SuperBowlWinners GROUP BY Winner HAVING Wins = "1";

SELECT Winner, Wins FROM SuperBowlWinners WHERE Wins = 6 OR Wins = 5 ORDER BY Wins DESC;

SELECT Winner, Wins,
CASE
    WHEN Wins = 1 THEN "5 Wins Needed"
    WHEN Wins = 2 THEN "4 Wins Needed"
    WHEN Wins = 3 THEN "3 Wins Needed"
    WHEN Wins = 4 THEN "2 Wins Needed"
    WHEN Wins = 5 THEN "1 Win Needed"
    WHEN Wins = 6 THEN "0 Wins Needed"
END AS "Needed for Most"
FROM SuperBowlWinners
ORDER BY "Needed for Most" ASC;
